...
pub fn flags_from_vec_safe(args: Vec<String>) -> clap::Result<Flags> {
  ...
  match matches.subcommand() {
    ...
    Some(("lsp", m)) => language_server_parse(&mut flags, m),
...
fn clap_root(version: &str) -> App {
  App::new("deno")
    ...
    .arg(
      Arg::new("unstable")
        .long("unstable")
        .about("Enable unstable features and APIs")
        .global(true),
    )
    .arg(
      Arg::new("log-level")
    ...
    .arg(
      Arg::new("quiet")
        .short('q')
        .long("quiet")
        .about("Suppress diagnostic output")
        .long_about(
...
fn runtime_args(app: App, include_perms: bool) -> App {
...
fn install_subcommand<'a>() -> App<'a> {
  runtime_args(App::new("install"), true)
...
fn eval_subcommand<'a>() -> App<'a> {
  runtime_args(App::new("eval"), false)
    .about("Eval script")
    .long_about(
...
Show documentation for runtime built-ins:
    ...
    .arg(
      Arg::new("json")
        .long("json")
        .about("Output documentation in JSON format")
        .takes_value(false),
    )
    .arg(
      Arg::new("private")
        .long("private")
        .about("Output private documentation")
        .takes_value(false),
    )
...
fn language_server_subcommand<'a>() -> App<'a> {
  App::new("lsp")
...
",
    ...
    .arg(
      Arg::new("ignore")
        ...
        .use_delimiter(true)
        .require_equals(true)
        .about("Ignore linting particular source files"),
    )
    .arg(
      Arg::new("json")
        .long("json")
        .about("Output lint result in JSON format")
        .takes_value(false),
    )
    .arg(
      Arg::new("files")
...
fn test_subcommand<'a>() -> App<'a> {
  runtime_args(App::new("test"), true)
    .setting(AppSettings::TrailingVarArg)
    .arg(
      Arg::new("no-run")
        .long("no-run")
    ...
    .arg(
      Arg::new("allow-none")
        .long("allow-none")
        .about("Don't return error code if no test files are found")
        .takes_value(false),
    )
    .arg(
      Arg::new("filter")
    ...
    .arg(
      Arg::new("coverage")
        ...
        .about("Collect coverage information"),
    )
    .arg(
      Arg::new("files")
...
fn lock_arg<'a>() -> Arg<'a> {
  Arg::new("lock")
    .long("lock")
    .value_name("FILE")
    .about("Check the specified lock file")
    .takes_value(true)
}
...
fn lock_write_arg<'a>() -> Arg<'a> {
  Arg::new("lock-write")
    .long("lock-write")
    .requires("lock")
    .about("Write lock file (use with --lock)")
}
...
fn import_map_arg<'a>() -> Arg<'a> {
  Arg::new("import-map")
    ...
    .value_name("FILE")
    .requires("unstable")
    .about("UNSTABLE: Load import map file")
    .long_about(
      "UNSTABLE:
